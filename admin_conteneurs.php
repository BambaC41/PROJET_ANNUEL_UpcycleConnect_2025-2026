...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['validate_demande_id'])) {
    ...
    db_safe_exec(function (PDO $pdo) use ($demandeId) {
        $stmt = $pdo->prepare('SELECT d.id_user, c.code AS conteneur_code, ca.code AS code_acces, cb.barcode_value, d.statut, d.titre
        ...
        $lines = [
            'Fiche de depot UpcycleConnect',
            'Demande #' . $demandeId,
            'Objet : ' . (string)($row['titre'] ?? ''),
            'Conteneur : ' . (string)($row['conteneur_code'] ?? ''),
            'Code acces : ' . (string)($row['code_acces'] ?? 'N/A'),
            'Valeur QR / code-barres : ' . ($payloadQr !== '' ? $payloadQr : 'N/A'),
            'Statut : validee',
        ];
        ...
        document_create_html((int)$row['id_user'], 'fiche_depot', 'Fiche depot #' . $demandeId, $html, null, $demandeId, null, $lines);
...
$demandesFiltered = array_values(array_filter($demandesDepot, static function ($d) use ($fStatut, $fCont, $fQ) {
    if ($fStatut !== 'all' && (($d['statut'] ?? '') !== $fStatut)) {
        return false;
    }
    if ($fCont > 0 && (int)($d['id_conteneur'] ?? 0) !== $fCont) {
        return false;
    }
    $needle = mb_strtolower((string)($d['titre'] ?? '') . ' ' . (string)($d['code_conteneur'] ?? '') . ' ' . (string)($d['id_user'] ?? ''));
    if ($fQ !== '' && !str_contains($needle, $fQ)) {
        return false;
    }
    return true;
}));
...
if ($editId > 0) {
    foreach ($conteneurs as $c) {
        if ((int)($c['id_conteneur'] ?? 0) === $editId) {
            $editRow = $c;
            break;
        }
    }
...
<section class="admin-content">
    <section class="admin-section">
        ...
        <?php if ($editRow !== null): ?>
            <h2 style="font-size:18px;">Modifier conteneur #<?= e((string)$editId) ?></h2>
            <form method="POST" class="form-grid" style="max-width:720px;">
                ...
                <div class="row-actions">
                    ...
                    <a class="btn-outline" href="admin_conteneurs.php">Fermer</a>
                </div>
            </form>
        <?php endif; ?>

        <h2 style="font-size:18px;margin-top:18px;">Créer un conteneur</h2>
            ...
            <?php foreach ($conteneurs as $c): ?>
                <tr>
                    ...
                    <td class="row-actions" style="flex-wrap:wrap;">
                        <a class="btn-outline" href="admin_conteneurs.php?edit=<?= (int)($c['id_conteneur'] ?? 0) ?>">Modifier</a>
    ...
    <section class="admin-section">
        ...
        <div class="table-wrap">
        <table class="admin-table">
            <thead><tr><th>ID</th><th>Objet</th><th>Utilisateur</th><th>Conteneur</th><th>Date</th><th>Statut</th><th>QR</th><th>Actions</th></tr></thead>
            <tbody>
