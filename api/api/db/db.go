...
func scanUser(scanner interface{ Scan(dest ...any) error }, user *model.User) error {
	return scanner.Scan(
		...
		&user.IsBanned, &user.BanReason, &user.BanUntil, &user.IsApproved,
	)
}
...
func GetUserByEmail(email string) (*model.User, error) {
	var user model.User
	err := scanUser(DB.QueryRow(userSelect+` WHERE email = ?`, email), &user)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func CreateUser(req model.RegisterRequest, passwordHash string) error {
	...
	_, err := DB.Exec(`
		INSERT INTO utilisateur (email, password_hash, pseudo, prenom, nom, photo_profil, bio, id_role, statut, is_approved)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func CreatePrestation(p model.Prestation) error {
	_, err := DB.Exec(`
		INSERT INTO prestation (titre, description, type, prix, is_active, id_categorie)
		VALUES (?, ?, ?, ?, ?, ?)
...
func CreateEvent(e model.Event) error {
	_, err := DB.Exec(`
		INSERT INTO session (date_debut, date_fin, lieu, capacite_max, statut, id_prestation, id_validateur)
		VALUES (?, ?, ?, ?, ?, ?, ?)
