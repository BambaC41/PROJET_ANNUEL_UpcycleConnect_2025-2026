...
db_safe_exec(function(PDO $pdo) use (&$totalRecuperations) {
    ...
    $stmt = $pdo->prepare('
        ...
        WHERE o.id_user = ? AND d.statut = "retiree"
    ');
    $stmt->execute([(int)$_SESSION['user_id']]);
    $totalRecuperations = (int)$stmt->fetchColumn();
    return true;
}, false);
...
$premiumCount = 0;
db_safe_exec(function(PDO $pdo) use (&$premiumCount) {
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM abonnement_pro WHERE id_pro = ? AND statut = "actif" AND date_fin >= CURDATE()');
    $stmt->execute([(int)$_SESSION['user_id']]);
...
if ($premiumCount > 0) {
    db_safe_exec(function(PDO $pdo) use (&$abonnement) {
        $stmt = $pdo->prepare('SELECT formule, date_debut, date_fin FROM abonnement_pro WHERE id_pro = ? AND statut = "actif" ORDER BY id_abonnement DESC LIMIT 1');
        $stmt->execute([(int)$_SESSION['user_id']]);
        $abonnementBDD = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($abonnementBDD) {
            $abonnement['formule'] = $abonnementBDD['formule'];
            $abonnement['date_debut'] = $abonnementBDD['date_debut'];
            $abonnement['date_fin'] = $abonnementBDD['date_fin'];
        }
        return true;
    }, false);
}
